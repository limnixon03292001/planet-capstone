...
const CommentButton = ({ post_id}) => {
    ...
    const { isLoading } = useQuery(['comment', post_id], getCommentPost, 
        {
            onSuccess: ({ data }) => {
  ...
  return (
    <>
        ...
        {isOpen &&
        <CommentModal closeModal={closeModal} comments={comments} setComments={setComments} isOpen={isOpen} post_id={post_id}/>}
    </>
  )
}
