...
const Sidebar = () => {
    ...
    socket?.on("messageReceived", (newMsgReceived) => {
      ...
      if (window.location.pathname !== newMsgReceived?.chatRoomLink) {
        ...
        return toast.custom((t) => (
          <Link
            to={`${newMsgReceived?.chatRoomLink}`}
            className="w-full block max-w-md"
          >
            <div
              className={`${
                  ...
                  <div className="ml-3 flex-1">
                    <p className="mt-1 text-sm text-gray-900">
                      ...
                      <span className="font-medium">
                        {" "}
                        {newMsgReceived?.msgContent?.firstname}{" "}
                        {newMsgReceived?.msgContent?.lastname}
                      </span>
                    </p>
                    <p className="text-xs">
                      <span className="font-medium">
  ...
  return (
    <div
      className="sm:visible sm:w-full sm:max-w-max sm:min-w-min invisible w-0 p-0 lg:max-w-[282px] xl:max-w-[370px] transition-all
      ...
      <div className="py-4 px-4 h-full w-full max-w-[280px] ml-auto overflow-y-auto flex flex-col">
        <div className="flex-shrink-0">
          ...
          <div>
            ...
            <div className="h-max mt-20">
              {linkNavigationBar?.slice(0, 2)?.map((data, id) => (
                ...
                >
                  {data?.icon}
                  <span
                    className="text-xl  self-end ml-3 font-[100]
                            hidden lg:block"
                  >
                    {data.title}
                  </span>
                ...
                >
                  <div className="relative">
                    ...
                    {chats?.filter(
                      (c) =>
                        c?.read === false &&
                        c?.sentby_id !== authUser?.user_id &&
                        c
                    )?.length !== 0 ? (
                      <span
                        className="bg-red-500 py-[2px] px-[6px] text-[11px] border-2 border-white rounded-full text-white absolute
                              -top-2 -right-2"
                      >
                        {
                          chats?.filter(
            ...
            <Link to={`/profile/${authUser?.user_id}`}>
              <div className="flex justify-start items-center my-10">
                <img
                  ...
                  className="w-[40px] h-[42px] rounded-full object-cover object-center lg:mr-2"
                />
                <div className="hidden lg:block">
                  <h1 className="font-extrabold block">
                    {authUser?.firstname} {authUser?.lastname}
                  </h1>
                  <p className="font-extralight text-xs text-gray-500 break-words">
                    {authUser?.email}
                  </p>
                </div>
              </div>
            ...
            <button
              className="flex items-center my-6 justify-center lg:justify-start"
              onClick={Logout}
            >
              <svg
              ...
              <span className="text-lg self-end ml-3 font-[100] hidden lg:block">
                Logout
              </span>
            </button>
          </div>
        ...
        {/* <div className="hidden lg:block mt-auto flex-shrink-0">
          <p className="text-gray-500 text-[10px] px-4 leading-normal">
            PLANeT Eco-trading: Platform for Malabonian Plant Enthusiast with
