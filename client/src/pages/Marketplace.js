...
import { getfilterPlantMarketplace, getPlantsMarketplace } from '../api/userApi';
...
const Marketplace = () => {
  ...
  // const { isLoading } = useQuery(['available-plants'], getPlantsMarketplace, {
  //   onSuccess: ({ data }) => {
  ...
  const { isLoading } = useQuery(['marketplace-plants', plantCat, searchData], getfilterPlantMarketplace, {
       onSuccess: ({ data }) => {
          // console.log("marketplace plants", data?.data);
          setmarketPlacePlants(data?.data);
