...
const MainMessages = ({ refetchAllChats }) => {
    ...
    const { isLoading: participantsLoading } = useQuery(['selected-room',id], getSelectedRoom,
    {
        retry: false,
        onSuccess: ({ data }) => {
    ...
    const check = (selectedRoom) => {
        if(authId === selectedRoom?.user_id) {
          return( <div className='flex items-center justify-start'>
                    <img src={selectedRoom?.fp} alt='profile' className="rounded-full w-9 h-9 md:w-12 md:h-12 object-cover object-center"/>
                    <div className='ml-2'>
                        <h1 className='text-md md:text-xl font-bold'>{selectedRoom?.ffn} {selectedRoom?.fln}</h1>
        ...
        if(authId === selectedRoom?.friend_id) {
            ...
            return ( <div className='flex items-center justify-start'>
                        <img src={selectedRoom?.userp} alt='profile' className="rounded-full w-9 h-9 md:w-12 md:h-12 object-cover object-center"/>
                        <div className='ml-2'>
                        <h1 className='text-md md:text-xl font-bold'>{selectedRoom?.userfn} {selectedRoom?.userln}</h1>
  ...
  return (
    <div className='mt-4 w-full h-full'>
        <div className='border-y border-gray-200 px-4 flex items-center justify-between z-10'>
            {participantsLoading ? 
                // Skeleton
                <div className='animate-pulse flex items-center py-5 w-full'>
                    <div className="w-9 h-9 md:w-12 md:h-12 mr-2 bg-slate-200 rounded-full"/>
                    
                    <div className='flex-1'>
                        <div className="h-3 w-full max-w-[140px] bg-slate-200 rounded-lg"/>
