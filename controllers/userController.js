...
exports.sendRecoveryLink = async (req, res) => {
  ...
  try {
    const user = await pool.query("SELECT * FROM user_acc WHERE email = $1", [
      email,
    ]);
    ...
    jwt.sign(
      ...
      (err, emailToken) => {
        if (err) {
          ...
          res.status(500).json({
            err: err?.message,
            errMsg: "error signing email to token",
          });
        }
        ...
        return res.status(200).json({
          message: "Account recovery link, successfully sent!",
          success: true,
        });
      }
    );
  } catch (error) {
    console.log("recovery link error:", error?.message);
...
exports.confirmationController = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "Your email confirmation has been successfully verified!",
    });
  } catch (error) {
    console.log(error?.message);
...
exports.registerController = async (req, res) => {
  ...
  try {
    ...
    const newUser = await pool.query(
      `INSERT INTO user_acc (firstname, lastname, email, password, phonenumber, baranggay, city, birthday, age, position) 
    ...
    await pool.query(`INSERT INTO acc_verify (acc_id) VALUES ($1)`, [
      newUser?.rows[0]?.user_id,
    ]);

    await pool.query(`INSERT INTO acc_list (acc_id) VALUES ($1)`, [
      newUser?.rows[0]?.user_id,
    ]);
  ...
  try {
    if (email) {
      ...
      if (user.rows[0]?.isverified === true) {
        return res
          .status(401)
          .json({ error: { email: `Email alredy verified!` } });
      }
...
exports.loginController = async (req, res) => {
  ...
  try {
    ...
    const isVerified = await pool.query(
      `SELECT * FROM acc_verify WHERE acc_id = $1`,
      [user.rows[0].user_id]
    );
    ...
    const isBlocked = await pool.query(
      "SELECT block FROM user_acc WHERE email = $1",
      [email]
    );

    if (isBlocked.rows[0]?.block === true) {
      return res.status(401).json({
        error: { blocked: true, message: `Your account has been blocked!` },
      });
    }
...
exports.updatePost = async (req, res) => {
  const { post_id, picture, description } = req.body;
...
exports.getPostLiked = async (req, res) => {
  ...
  try {
    const rows = await pool.query(
      "SELECT * FROM posts_like WHERE user_id = $1 AND post_id = $2",
      [user_id, post_id]
    );
...
exports.postLike = async (req, res) => {
  ...
  try {
    await pool.query(
      "INSERT INTO posts_like (post_id, user_id) VALUES ($1, $2) RETURNING *",
      [post_id, user_id]
    );
    ...
    res
      .status(201)
      .json({ likePost: rows.rows[0], message: "liked successfully" });
  } catch (error) {
    console.log(error?.message);
    return res.status(500).json({
      error: error?.message,
...
exports.postDislike = async (req, res) => {
  ...
  try {
    await pool.query(
      "DELETE FROM posts_like WHERE post_id = $1 AND user_id = $2",
      [post_id, user_id]
    );
    // console.log(rows.rows);
    const rows = await pool.query(
    ...
    res
      .status(201)
      .json({ dislikePost: rows.rows[0], message: "disliked successfully" });
  } catch (error) {
    console.log(error?.message);
    return res.status(500).json({
      error: error?.message,
...
exports.followUser = async (req, res) => {
  ...
  try {
    ...
    if (followerUserId === userId) {
      return res
        .status(500)
        .json({ error: "Following yourself is not allowed!" });
    }
    ...
    res
      .status(200)
      .json({ followed: rows?.rows[0], message: "followed sucessfully!" });
  } catch (error) {
    console.log("x", error?.message);
    return res.status(500).json({
      error: error?.message,
...
exports.unFollowUser = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ unfollowed: rows?.rows[0], message: "unfollowed sucessfully!" });
  } catch (error) {
    console.log("x", error?.message);
    return res.status(500).json({
      error: error?.message,
...
exports.getIsFollowingUser = async (req, res) => {
  ...
  try {
    const rows = await pool.query(
      "SELECT * FROM user_followers WHERE user_id = $1 AND followers_user_id = $2",
      [userId, followerUserId]
    );
...
exports.addPlantCollection = async (req, res) => {
  ...
  try {
    ...
    const queryPlantDetails = ` 
        ...
        `;
    const resPlantDetails = await client.query(queryPlantDetails, [
      authUserId,
      data?.plantDetails?.plantName,
      data?.plantDetails?.desc,
      data?.plantDetails?.plantCat,
      data?.plantDetails?.datePlanted,
      plant_img,
    ]);
    ...
    const queryGrowingPref = `
            ...
            VALUES ($1, $2 ,$3, $4, $5, $6)
        `;
    await client.query(queryGrowingPref, [
      idPlantDetail,
      sunPref,
      interLight,
      soilPref,
    ...
    await client.query(queryGrowingInformation, [
      idPlantDetail,
      data?.growingInfo?.avgHeight,
      data?.growingInfo?.avgWidth,
      data?.growingInfo?.foliageColor,
      data?.growingInfo?.foliageType,
      data?.growingInfo?.foliageScent,
      data?.growingInfo?.flowerColor,
      data?.growingInfo?.fragrant,
      data?.growingInfo?.nocturnalFlow,
      data?.growingInfo?.repeatBloom,
      data?.growingInfo?.floweringPer,
    ]);
    ...
    return res
      .status(201)
      .json({ message: "Added plant successfully into your collections! " });
  } catch (error) {
...
exports.filterPlantCollections = async (req, res) => {
  ...
  try {
      ...
      ` 
            ...
            ${
              category && category !== "All"
                ? `AND to_tsvector(cpd.user_id || ' ' || cpd.category) @@ plainto_tsquery(%L)`
                : `AND NOT to_tsvector(cpd.user_id || ' ' || cpd.category) @@ plainto_tsquery(%L)`
            }
            
            ${
              sunPref
                ? `AND to_tsvector(cpd.user_id || ' ' || cgp.sun_pref) @@ plainto_tsquery(%L)`
                : `AND NOT to_tsvector(cpd.user_id || ' ' || cgp.sun_pref) @@ plainto_tsquery(%L)`
            }

            ${
              interLight
                ? `AND to_tsvector(cpd.user_id || ' ' || cgp.inter_light) @@ plainto_tsquery(%L)`
                : `AND NOT to_tsvector(cpd.user_id || ' ' || cgp.inter_light) @@ plainto_tsquery(%L)`
            }
            
            ${
              soilPref
                ? `AND to_tsvector(cpd.user_id || ' ' || cgp.soil_pref) @@ plainto_tsquery(%L)`
                : `AND NOT to_tsvector(cpd.user_id || ' ' || cgp.soil_pref) @@ plainto_tsquery(%L)`
            }
            
            ${
              waterReq
                ? `AND to_tsvector(cpd.user_id || ' ' || cgp.water_req) @@ plainto_tsquery(%L)`
                : `AND NOT to_tsvector(cpd.user_id || ' ' || cgp.water_req) @@ plainto_tsquery(%L)`
            }

            ${
              nativeHab
                ? `AND to_tsvector(cpd.user_id || ' ' || cgp.native_habitat) @@ plainto_tsquery(%L)`
                : `AND NOT to_tsvector(cpd.user_id || ' ' || cgp.native_habitat) @@ plainto_tsquery(%L)`
            }

            ${
              search
                ? `AND to_tsvector(cpd.user_id || ' ' || cpd.* || cgp.*) @@ plainto_tsquery(%L)`
    ...
    const count = await pool.query(
      `SELECT COUNT(cpd.plant_detail_id) FROM coll_plant_details cpd  WHERE cpd.user_id = $1`,
      [Number(userId)]
    );
