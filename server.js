...
const server = app.listen(PORT, () => {
  console.log(
    `The server is running at localhost ${PORT}, 'node=',${process.env.NODE_ENV}`
  );
});
...
io.on("connection", (socket) => {
  ...
  socket.on("sendMessage", (newMessageReceived) => {
    ...
    users.forEach((user) => {
      // console.log("socket:",)
      socket.to(user[0]?.socketId).emit("messageReceived", {
        msgContent: newMessageReceived?.data?.msg,
        chatRoomLink: newMessageReceived?.data?.chatRoomLink,
